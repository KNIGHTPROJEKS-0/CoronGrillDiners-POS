...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Failed to fetch void codes" },
      { status: 500 },
    );
  }
}
...
export async function PUT() {
  ...
  try {
    ...
    try {
      await client.query(
        `CREATE UNIQUE INDEX IF NOT EXISTS idx_void_codes_code_unique ON void_codes(code)`,
      );
    } catch {
      /* duplicate codes could prevent a unique index; keep going */
    }

    const actor = {
      id:
        session.user.id ?? session.user.email ?? session.user.name ?? "unknown",
      username:
    ...
    return NextResponse.json(
      { error: "Failed to generate codes", details: String(error) },
      { status: 500 },
    );
  } finally {
    client.release();
...
export async function POST(request: Request) {
  ...
  if (!code || !saleId) {
    return NextResponse.json(
      { error: "code and saleId are required" },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    const codeResult = await client.query(
      `SELECT id, used_at FROM void_codes WHERE code = $1 FOR UPDATE`,
      [normalizedCode],
    );

    if (codeResult.rows.length === 0 || codeResult.rows[0].used_at) {
      await client.query("ROLLBACK");
      return NextResponse.json(
        {
          error:
            "Invalid or already-used void code. Please request a new code from admin.",
        },
        { status: 400 },
      );
    }
    ...
    const saleItems: Array<{ id?: number; quantity: number }> =
      itemsResult.rows[0]?.items ?? [];
    ...
    const consumeResult = await client.query(
      `UPDATE void_codes
       ...
       RETURNING id`,
      [actor.username, saleId, codeResult.rows[0].id],
    );
    ...
    if (consumeResult.rows.length === 0) {
      ...
      return NextResponse.json(
        {
          error:
            "Void code was already used. Please request a new code from admin.",
        },
        { status: 409 },
      );
    }
