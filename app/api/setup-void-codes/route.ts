...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: 'void_codes table created successfully' })
  } catch (error) {
    await client.query('ROLLBACK')
    console.error('Failed to create void_codes table:', error)
    return NextResponse.json({ error: 'Failed to create void_codes table', details: String(error) }, { status: 500 })
  } finally {
    client.release()
  }
}
