...
import {
  FileText, Printer, RefreshCw, AlertTriangle, Clock,
  CheckCircle, XCircle, Ban, Wallet, CreditCard, ChevronDown,
  ChevronUp, Calendar, User, Receipt,
} from "lucide-react"
...
function itemsSummary(items: SaleRecord["items"]) {
  ...
  const top = items.slice(0, 2).map(i => `${i.quantity}× ${i.name}`).join(", ")
...
function printReport(shift: ShiftWithStats, sales: SaleRecord[]) {
  ...
  const rowsHtml = sales.map((s, i) => {
    const statusClass =
      s.status === "void" ? "color:#dc2626;font-weight:600" :
      s.status === "cancelled" ? "color:#d97706;font-weight:600" :
      "color:#16a34a;font-weight:600"
    const statusLabel = s.status === "void" ? "VOID" : s.status === "cancelled" ? "CANCELLED" : "COMPLETED"
    const itemsText = Array.isArray(s.items)
      ? s.items.map(it => `${it.quantity}× ${it.name}`).join(", ")
      : "—"
    const voidNote = s.void_reason ? `<br><span style="color:#888;font-size:8pt">Reason: ${s.void_reason}</span>` : ""
    return `
...
export default function SalesSection() {
  ...
  const [dateFrom, setDateFrom] = useState(sevenDaysAgo.toLocaleDateString("en-CA", { timeZone: "Asia/Manila" }))
  const [dateTo, setDateTo]     = useState(today.toLocaleDateString("en-CA", { timeZone: "Asia/Manila" }))
  ...
  const fetchShifts = useCallback(async () => {
    ...
    setFetchError(false)
    try {
      const res = await fetch(`/api/sales/shifts?from=${dateFrom}&to=${dateTo}`)
  ...
  const totalCompleted = filtered.reduce((a, s) => a + (s.completed_total || 0), 0)
  const totalOrders    = filtered.reduce((a, s) => a + (s.completed_count || 0), 0)
      ...
      <div className="bg-white rounded-xl border shadow-sm p-4">
        <div className="flex flex-wrap gap-3 items-end">
          ...
          <div className="flex flex-col gap-1 flex-1 min-w-[160px]">
            <label className="text-xs font-medium text-muted-foreground">Search Cashier</label>
            <input
              type="text"
      ...
      {!loading && !fetchError && filtered.length > 0 && (
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-white rounded-xl border shadow-sm p-4 text-center">
            <p className="text-xs text-muted-foreground uppercase tracking-wide mb-1">Shifts</p>
            <p className="text-2xl font-bold">{filtered.length}</p>
          </div>
          <div className="bg-white rounded-xl border shadow-sm p-4 text-center">
            <p className="text-xs text-muted-foreground uppercase tracking-wide mb-1">Completed Orders</p>
            <p className="text-2xl font-bold">{totalOrders}</p>
          </div>
          <div className="bg-white rounded-xl border shadow-sm p-4 text-center">
            <p className="text-xs text-muted-foreground uppercase tracking-wide mb-1">Total Sales</p>
            <p className="text-2xl font-bold text-green-700">{fmt(totalCompleted)}</p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl border shadow-sm">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
            ...
            <p className="text-sm mt-1">Try a different date range or cashier name.</p>
          </div>
        ) : (
                    ...
                    <div className="flex items-start justify-between gap-4 flex-wrap">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap mb-1">
                          ...
                          {shift.archived && (
                            <Badge variant="outline" className="text-[10px] h-4 px-1.5">Archived</Badge>
                          )}
                        </div>
                        <div className="flex items-center gap-3 text-xs text-muted-foreground flex-wrap">
                          ...
                          <span className="flex items-center gap-1">
                            ...
                            {fmtTime(shift.start_time)} – {shift.end_time ? fmtTime(shift.end_time) : "Ongoing"}
                          </span>
                          <span>({durationStr(shift.start_time, shift.end_time)})</span>
                        </div>
                      ...
                      <div className="flex flex-col items-end gap-2 shrink-0">
                        <div className="text-right">
                          ...
                          <p className="text-lg font-bold text-green-700">{fmt(shift.completed_total)}</p>
                        </div>
                    ...
                    {isExpanded && (
                      <div className="mt-3 pt-3 border-t grid grid-cols-2 sm:grid-cols-4 gap-3">
                        <div className="bg-gray-50 rounded-lg p-3">
                          ...
                          <p className="font-semibold">{fmt(shift.start_balance)}</p>
                        </div>
                        <div className="bg-gray-50 rounded-lg p-3">
                          <p className="text-xs text-muted-foreground">Expected Cash</p>
                          <p className="font-semibold">{shift.expected_cash !== null ? fmt(shift.expected_cash) : "—"}</p>
                        </div>
                        <div className="bg-gray-50 rounded-lg p-3">
                          <p className="text-xs text-muted-foreground">Actual Cash</p>
                          <p className="font-semibold">{shift.end_balance !== null ? fmt(shift.end_balance) : "—"}</p>
                        </div>
                        <div className={`rounded-lg p-3 ${
                          disc < 0 ? "bg-red-50" : disc > 0 ? "bg-green-50" : "bg-gray-50"
                        }`}>
                          <p className="text-xs text-muted-foreground">Discrepancy</p>
                          <p className={`font-semibold ${disc < 0 ? "text-red-600" : disc > 0 ? "text-green-700" : ""}`}>
                            {shift.discrepancy !== null ? `${disc < 0 ? "-" : disc > 0 ? "+" : ""}${fmt(Math.abs(disc))}` : "—"}
                          </p>
                        </div>
                        {shift.notes && (
                          <div className="col-span-full bg-amber-50 border border-amber-200 rounded-lg p-3">
                            ...
                            <p className="text-sm text-amber-900">{shift.notes}</p>
                          </div>
                        )}
                      </div>
                    )}
            ...
            <DialogTitle className="flex items-center gap-2">
              ...
              {reportShift && (
                <span className="text-sm font-normal text-muted-foreground ml-1">
                  — {reportShift.cashier_name} · {fmtDate(reportShift.start_time)}
                </span>
              )}
            </DialogTitle>
          </DialogHeader>
          ...
          {reportLoading ? (
            ...
            <div className="flex items-center justify-center py-16 text-center px-6">
              <div>
                ...
                <Button variant="outline" size="sm" onClick={() => reportShift && openReport(reportShift)}>
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto px-6 py-4 space-y-5">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 p-4 bg-gray-50 rounded-xl border">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">@{reportShift.cashier_username}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground font-medium mb-0.5">Shift Period</p>
                    <p className="font-semibold">{fmtDate(reportShift.start_time)}</p>
                    <p className="text-xs text-muted-foreground">
                      {fmtTime(reportShift.start_time)} – {reportShift.end_time ? fmtTime(reportShift.end_time) : "Ongoing"}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground font-medium mb-0.5">Duration</p>
                    <p className="font-semibold">{durationStr(reportShift.start_time, reportShift.end_time)}</p>
                    <p className="text-xs text-muted-foreground">
                      {reportShift.status === "open" ? "Shift still active" : "Shift closed"}
                    </p>
                  </div>
                </div>
                  ...
                  {[
                    {
                      ...
                      count: reportSales.filter(s => !s.status || s.status === "completed").length,
                      total: reportSales.filter(s => !s.status || s.status === "completed").reduce((a, s) => a + s.grand_total, 0),
                      cls: "border-green-200 bg-green-50 text-green-800",
                    },
                    {
                      label: "Voided", icon: XCircle,
                      count: reportSales.filter(s => s.status === "void").length,
                      total: reportSales.filter(s => s.status === "void").reduce((a, s) => a + s.grand_total, 0),
                      cls: "border-red-200 bg-red-50 text-red-800",
                    },
                    {
                      label: "Cancelled", icon: Ban,
                      count: reportSales.filter(s => s.status === "cancelled").length,
                      total: 0,
                      cls: "border-amber-200 bg-amber-50 text-amber-800",
                    },
                    {
                      label: "Total Sales", icon: Receipt,
                      count: reportSales.filter(s => !s.status || s.status === "completed").length,
                      total: reportSales.filter(s => !s.status || s.status === "completed").reduce((a, s) => a + s.grand_total, 0),
                    ...
                    <div key={label} className={`rounded-xl border p-4 text-center ${cls}`}>
                      ...
                      <p className={`font-bold mt-1 ${big ? "text-xl" : "text-2xl"}`}>{count}</p>
                ...
                <div>
                  <h3 className="font-semibold text-sm mb-2 flex items-center gap-2">
                    ...
                    <span className="text-xs font-normal text-muted-foreground">({reportSales.length} transaction{reportSales.length !== 1 ? "s" : ""})</span>
                  </h3>
                      ...
                      <table className="w-full text-sm">
                        <thead className="bg-gray-900 text-white">
                          <tr>
                            ...
                            <th className="px-3 py-2 text-left text-xs font-medium">Order No.</th>
                            <th className="px-3 py-2 text-left text-xs font-medium">Items</th>
                          ...
                          {reportSales.map((s, i) => (
                            <tr key={s.id} className={`${i % 2 === 0 ? "bg-white" : "bg-gray-50"} hover:bg-blue-50/40`}>
                              ...
                              <td className="px-3 py-2 text-xs text-muted-foreground max-w-[200px]">
                                ...
                                {s.void_reason && (
                                  <span className="block text-red-500 text-[10px] mt-0.5">↳ {s.void_reason}</span>
                                )}
                              </td>
                              ...
                              <td className="px-3 py-2 text-center">
                                <Badge
                                  className={`text-[10px] h-4 px-1.5 border-0 ${
                                    ...
                                    s.status === "cancelled" ? "bg-amber-100 text-amber-700" :
                                    "bg-green-100 text-green-700"
                                  }`}
                                >
                                  {s.status === "void" ? "Void" : s.status === "cancelled" ? "Cancelled" : "Completed"}
                                </Badge>
                              </td>
                            </tr>
                          ))}
                        ...
                        <tfoot>
                          <tr className="bg-gray-100 border-t-2 border-gray-900">
                            <td colSpan={5} className="px-3 py-2 text-sm font-bold text-right">
                              Total Completed Sales:
                            </td>
                            <td className="px-3 py-2 text-sm font-bold text-right text-green-700">
                              {fmt(reportSales.filter(s => !s.status || s.status === "completed").reduce((a, s) => a + s.grand_total, 0))}
                            </td>
                            <td />
                          </tr>
                        </tfoot>
                ...
                {/* Cash reconciliation */}
                <div>
                  <h3 className="font-semibold text-sm mb-2">Cash Reconciliation</h3>
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      { label: "Starting Cash Balance",  val: fmt(reportShift.start_balance) },
                      { label: "Total Cash Sales",       val: fmt(reportShift.total_cash_sales) },
                      { label: "Expected Cash on Hand",  val: reportShift.expected_cash !== null ? fmt(reportShift.expected_cash) : "—" },
                      { label: "Actual Cash Counted",    val: reportShift.end_balance !== null ? fmt(reportShift.end_balance) : "— (Shift Open)" },
                    ].map(({ label, val }) => (
                      <div key={label} className="bg-gray-50 rounded-lg border p-3 flex justify-between items-center">
                        <span className="text-sm text-muted-foreground">{label}</span>
                        <span className="font-semibold">{val}</span>
                      </div>
                    ))}
                    <div className={`col-span-full rounded-lg border p-3 flex justify-between items-center font-semibold ${
                      (reportShift.discrepancy ?? 0) < 0 ? "bg-red-50 border-red-200 text-red-700" :
                      (reportShift.discrepancy ?? 0) > 0 ? "bg-green-50 border-green-200 text-green-700" :
                      "bg-gray-50"
                    }`}>
                      <span>
                        Discrepancy
                        {reportShift.discrepancy !== null && (
                          <span className="ml-2 text-xs font-medium opacity-80">
                            {(reportShift.discrepancy ?? 0) < 0 ? "SHORT" : (reportShift.discrepancy ?? 0) > 0 ? "OVER" : "BALANCED"}
                          </span>
                        )}
                      </span>
                      <span>
                ...
                {reportShift.notes && (
                  <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
                    <p className="text-xs font-semibold text-amber-800 mb-1">Notes / Remarks</p>
                    <p className="text-sm text-amber-900">{reportShift.notes}</p>
                  </div>
                )}
              </div>
              ...
              <div className="px-6 py-4 border-t shrink-0 bg-gray-50 flex items-center justify-between gap-3 flex-wrap">
                <p className="text-xs text-muted-foreground">
                  Ref: SHIFT-{String(reportShift.id).padStart(6, "0")} &bull; Generated {fmtDateTime(new Date().toISOString())}
                </p>
                <div className="flex gap-2">
