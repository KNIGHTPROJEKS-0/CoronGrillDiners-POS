...
export const PRINTER_NAMES: Record<PrinterRole, string> = {
  cashier: "RPP02N_1", // cashier receipt printer
  kitchen: "RPP02N_2", // kitchen ticket printer
};
...
export async function connectUSB(role: PrinterRole): Promise<void> {
  if (!("serial" in navigator)) {
    throw new Error(
      "Web Serial API not supported. Use Chrome 89+ on desktop (not Safari or Firefox).",
    );
  }
...
export async function autoReconnectBluetooth(
  role: PrinterRole,
): Promise<boolean> {
  const nav = navigator as any;
...
export async function connectBluetooth(role: PrinterRole): Promise<void> {
  if (!("bluetooth" in navigator)) {
    throw new Error(
      "Web Bluetooth API not supported. Use Chrome on Android or Chrome desktop (not Safari or Firefox).",
    );
  }
  const nav = navigator as any;
...
export async function printTo(
  ...
  if (char) {
    try {
      ...
      console.warn(
        `[printer] BLE write failed for ${role}, attempting reconnect…`,
        writeErr,
      );
      delete btChars[role];
      status[role] = { connected: false, name: "", type: null };
...
export function saveRawBTPrinter(role: PrinterRole, printerName: string) {
  try {
    localStorage.setItem(`cgd_rawbt_${role}`, printerName);
  } catch {
    /* SSR */
  }
}
...
export function loadRawBTPrinter(role: PrinterRole): string | null {
  try {
    return localStorage.getItem(`cgd_rawbt_${role}`);
  } catch {
    return null;
  }
}
...
export function clearRawBTPrinter(role: PrinterRole) {
  try {
    localStorage.removeItem(`cgd_rawbt_${role}`);
  } catch {
    /* SSR */
  }
}
...
export async function printToRawBT(
  role: PrinterRole,
  data: Uint8Array,
): Promise<boolean> {
...
function metaKey(role: PrinterRole) {
  return `cgd_printer_${role}`;
}

function saveMeta(role: PrinterRole, type: ConnType, name: string) {
  try {
    localStorage.setItem(metaKey(role), JSON.stringify({ type, name }));
  } catch {
    /* SSR */
  }
}
...
function clearMeta(role: PrinterRole) {
  try {
    localStorage.removeItem(metaKey(role));
  } catch {
    /* SSR */
  }
}
